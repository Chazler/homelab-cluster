...
Jellyfin and Jellyseerr are published through Envoy Gateway. Jellystat, Sonarr,
Sonarr Anime, Radarr, Prowlarr, Bazarr, and SABnzbd are also published, but
their HTTPRoutes carry the `authentik-forward-auth: "true"` label. The
cluster Kyverno policy generates a fail-closed Envoy external-auth
`SecurityPolicy`; Authentik applies each hostname's own provider and
application policy before forwarding requests to those administrative services.
Jellyfin and Jellyseerr use their own application-level authentication.
